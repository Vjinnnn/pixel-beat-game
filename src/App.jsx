    ...
    <div style={{ height: '100vh', width: '100%', backgroundColor: currentTheme.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#4E342E', fontFamily: '"VT323", monospace', userSelect: 'none', overflow: 'hidden' }}>
      ...
      {!gameStarted ? (
        ...
        <div style={{ position: 'relative', zIndex: 2 }}>
          {/* HUD & PAUSE BUTTON */}
          <div style={{ width: '420px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '15px', background: '#FFF', border: `5px solid ${currentTheme.theme}`, marginBottom: '10px', borderRadius: '8px' }}>
            <span style={{ fontSize: '1.8rem', color: '#E57373' }}>❤ {lives}</span>
            <span style={{ fontSize: '2rem', fontWeight: 'bold' }}>{score}</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
               <span style={{ fontSize: '1.8rem', color: currentTheme.theme }}>x{combo}</span>
               {/* УТАСАН ДЭЭР ПАУЗ АВАХ ТОВЧ */}
               <button onClick={() => togglePause()} style={{ background: currentTheme.theme, color: '#FFF', border: 'none', borderRadius: '4px', padding: '5px 12px', cursor: 'pointer', fontSize: '1.2rem' }}>
                 {isPaused ? '▶' : '⏸'}
               </button>
            </div>
          </div>
          ...
          <div style={{ position: 'relative', width: '420px', height: '620px', backgroundColor: 'rgba(255, 255, 255, 0.95)', border: `6px solid ${currentTheme.theme}`, overflow: 'hidden', borderRadius: '10px', touchAction: 'none' }}>
            ...
            {/* --- ШИНЭЧЛЭГДСЭН PAUSE OVERLAY --- */}
            {isPaused && (
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(255, 255, 255, 0.96)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}>
                <h2 style={{ fontSize: '3.5rem', color: currentTheme.theme, marginBottom: '25px' }}>PAUSED</h2>
                <button onClick={() => togglePause(false)} style={{ padding: '15px 50px', background: currentTheme.theme, color: '#FFF', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '1.8rem', marginBottom: '15px' }}>RESUME</button>
                <button onClick={handleBackToMenu} style={{ background: 'none', border: `3px solid ${currentTheme.theme}`, color: currentTheme.theme, cursor: 'pointer', padding: '10px 20px', borderRadius: '8px', fontSize: '1.2rem' }}>BACK TO MENU</button>
              </div>
            )}
            
            {(gameWin || gameOver) && (
              <div style={{ position: 'absolute', inset: 0, background: gameWin ? '#FFF' : '#FFEBEE', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
